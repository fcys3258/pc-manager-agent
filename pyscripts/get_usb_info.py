TOOL_DEF = {"name": "get_usb_info", "description": "获取USB设备信息", "input_schema": {"type": "object", "properties": {}, "required": []}}
...
def get_usb_info(
    dry_run: bool = False,
    # 使用 r"" 原始字符串避免路径转义问题
    script_path: str = r"scripts\scripts\powershell\get_usb_info.ps1"
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    target_script = r"scripts\scripts\powershell\get_usb_info.ps1"
    print("--- Test: Check USB Storage Devices ---")
    
    if not os.path.exists(target_script):
        print(f"[警告] 找不到脚本: {target_script}")
    
    res = get_usb_info(script_path=target_script)
    
    if res.get("ok"):
        devices = res['data']['usb_storage_devices']
